...
def tensor_from_contours(lq_tensor):
    """Convert grayscale L tensor (B, 1, H, W) into binary contour-only image."""
    contour_images = []
    for i in range(lq_tensor.shape[0]):
        # Convert tensor to uint8 image
        img_np = lq_tensor[i, 0].detach().cpu().numpy()
        img_np = np.clip(img_np * 255.0, 0, 255).astype(np.uint8)

        # Blur to smooth out small noise
        img_np = cv2.GaussianBlur(img_np, (5, 5), 0)

        # Threshold to binary (optional but helps)
        ...
        contour_canvas = np.zeros_like(img_np)
        cv2.drawContours(contour_canvas, contours, -1, (255,), 1)

        # Convert back to torch tensor
        contour_tensor = torch.from_numpy(contour_canvas.astype(np.float32) / 255.0).unsqueeze(0)
